...
impl FileLevelRule for NoCrossModuleOptionReads {
    fn code(&self) -> u32 { 118 }
    fn name(&self) -> &'static str { "no-cross-module-option-reads" }
    fn severity(&self) -> Severity { Severity::Error }
    fn note(&self) -> &'static str { "Module reads config from namespace not declared as options in this file." }
    ...
    fn validate_file(&self, path: &Path, content: &str) -> Option<FileLevelReport> {
        ...
        for cap in config_read_re.captures_iter(content) {
            ...
            if !declared_set.contains(ns) {
                return Some(FileLevelReport {
                    ...
                    message: format!("Module reads config.{} but does not declare options.{} in this file.", ns, ns),
                    note: self.note(),
                    code: self.code(),
        ...
        let assert_config_re = Regex::new(r"assert\s+.*?([^a-zA-Z0-9_.])config\.([a-zA-Z_]\w*)\.").unwrap();
        for cap in assert_config_re.captures_iter(content) {
            let ns = cap.get(2)?.as_str();
            if !declared_set.contains(ns) {
                return Some(FileLevelReport {
                    file: path.to_string_lossy().into_owned(),
                    message: format!("Module asserts on config.{} but does not declare options.{} in this file.", ns, ns),
                    note: self.note(),
                    code: self.code(),
                    severity: self.severity(),
                });
